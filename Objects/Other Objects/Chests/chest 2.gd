...
func _process(delta: float) -> void:
	if player_in_range and Input.is_action_just_pressed("ui_accept") and GlobalInfo.has_key_2:
		animated_sprite.play("Open")
		GlobalInfo.gold += 25
		GlobalInfo.has_key_2 = false
		$RichTextLabel.text = "You found 25 gold!"
		await get_tree().create_timer(5).timeout
	if player_in_range and Input.is_action_just_pressed("ui_accept") and (GlobalInfo.has_key_1 or GlobalInfo.has_key_3 or GlobalInfo.has_key_4):
